...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!wellKnown) {
    return NextResponse.json(
      { data: null, error: { code: 'OFFLINE', message: 'Server unreachable.', status: 503 }, time: Date.now(), request: '/api/auth/register' },
      { status: 503 },
    );
  }

  const backendUrl = `${wellKnown.gateway.api.replace(/\/$/, '')}/api/auth/register`;
